...
function SubscriptionForm(props) {
    ...
    const handleSubmit = (e) =>{
      ...
      if(formData.no_of_user_subscriber.trim() === "" || formData.no_of_user_subscriber <=0){
  ...
  return (
    <div className='container'>
        <h3 className='text-uppercase'> Add Subscription</h3>
        <form onSubmit={handleSubmit}>
            ...
            <label htmlFor='noOfUsers'>Number of Users:</label>
                <input type={"number"}
                  className="form-control"
                  name='no_of_user_subscriber'
                  value={formData.no_of_user_subscriber}
                  placeholder="Enter number of users"
                  onChange={(e)=>setFormData({...formData,no_of_user_subscriber:e.target.value})}
                  required
                  />
            </div>
